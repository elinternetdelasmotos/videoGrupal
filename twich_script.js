      ...
      if(mensaje_de_chat.getElementsByClassName("chat-author__display-name").length>0){
        try {
            ...
            for (let imagen of emisor_tag) {
               ...
               if (imagen.getAttribute("alt")=="Prime Gaming"){autor=emisor;}
          };
...
chrome.runtime.onMessage.addListener(
  function(request, sender, sendResponse) {
    if (request.greeting == "activa_lector_chat_twich")
    {
      preparatwich();
      sendResponse({farewell: "hecho twich"});
    };
